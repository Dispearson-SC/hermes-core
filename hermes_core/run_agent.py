...
try:
    import hermes_core.hermes_bootstrap  # noqa: F401
...
def _gateway_origin_json(agent: "AIAgent") -> Optional[str]:
    ...
    if not profile:
        try:
            from hermes_core.runtime.profiles import get_active_profile_name
...
from hermes_core.agent.iteration_budget import IterationBudget
from hermes_core.runtime.env_loader import load_hermes_dotenv
from hermes_core.runtime.timeouts import get_provider_request_timeout, get_provider_stale_timeout
...
from hermes_core.model_tools import get_toolset_for_tool
...
from hermes_core.tools.interrupt import set_interrupt as _set_interrupt
def cleanup_browser(task_id=None):
    """No browser session to close."""
    return None


from hermes_core.agent.memory_provider import is_trivial_prompt
from hermes_core.agent.client_lifecycle import ClientLifecycleMixin
from hermes_core.agent.stream_delivery import StreamDeliveryMixin
from hermes_core.agent.status_output import StatusOutputMixin
from hermes_core.agent.api_request_hooks import ApiRequestHooksMixin
from hermes_core.agent.api_error_summary import ApiErrorSummaryMixin
from hermes_core.agent.interrupt_control import InterruptControlMixin
from hermes_core.agent.turn_explainers import TurnExplainersMixin
from hermes_core.agent.activity_tracking import ActivityTrackingMixin
from hermes_core.agent.rate_limit_credits import RateLimitCreditsMixin
from hermes_core.agent.session_persistence import SessionPersistenceMixin
from hermes_core.agent.compression_facade import CompressionFacadeMixin
from hermes_core.agent.turn_facade import TurnFacadeMixin
from hermes_core.agent.vision_message_prep import VisionMessagePrepMixin
from hermes_core.agent.reasoning_params import ReasoningParamsMixin
from hermes_core.agent.lazy_forward import forward as _forward, forward_static as _forward_static
from hermes_core.agent.session_activity import ActivityProvenance
from hermes_core.agent.model_metadata import is_local_endpoint
from hermes_core.agent.message_sanitization import (
...
from hermes_core.agent.codex_responses_adapter import (
    _derive_responses_function_call_id as _codex_derive_responses_function_call_id,
    _split_responses_tool_id as _codex_split_responses_tool_id,
    _summarize_user_message_for_log,
)
from hermes_core.agent.tool_guardrails import ToolGuardrailDecision, append_toolguard_guidance, toolguard_synthetic_result
from hermes_core.utils import base_url_host_matches, base_url_hostname, env_float, model_forces_max_completion_tokens
...
def _review_should_defer(agent: Any, task_cfg: Optional[Dict[str, Any]]) -> bool:
    ...
    from hermes_core.agent.review_idle_queue import defer_mode, review_targets_managed_local
...
class AIAgent(
    ...
    def __init__(
        ...
        from hermes_core.agent.agent_init import init_agent
    ...
    def _get_session_db_for_recall(self):
        ...
        try:
            from hermes_core.seams.session_state import acquire
    ...
    def _session_row_model_config(self) -> Any:
        ...
        try:
            from hermes_core.tools.approval import is_session_yolo_enabled
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from hermes_core.runtime.models_local import ensure_lmstudio_model_loaded
    ...
    switch_model = _forward("hermes_core.agent.agent_runtime_helpers", "switch_model")
    ...
    _stream_diag_init = _forward_static("hermes_core.agent.stream_diag", "stream_diag_init")
    _stream_diag_capture_response = _forward("hermes_core.agent.stream_diag", "stream_diag_capture_response")
    _flatten_exception_chain = _forward_static("hermes_core.agent.stream_diag", "flatten_exception_chain")
    ...
    _log_stream_retry = _forward("hermes_core.agent.stream_diag", "log_stream_retry")
    _emit_stream_drop = _forward("hermes_core.agent.stream_diag", "emit_stream_drop")
    ...
    _check_compression_model_feasibility = _forward("hermes_core.agent.conversation_compression", "check_compression_model_feasibility")
    _replay_compression_warning = _forward("hermes_core.agent.conversation_compression", "replay_compression_warning")
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        from hermes_core.agent.reasoning_timeouts import get_reasoning_stale_timeout_floor
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from hermes_core.agent.chat_completion_helpers import estimate_request_context_tokens
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        if self.api_mode != "codex_responses":
            return None
        from hermes_core.agent.codex_responses_adapter import classify_responses_route
    ...
    _anthropic_prompt_cache_policy = _forward("hermes_core.agent.agent_runtime_helpers", "anthropic_prompt_cache_policy")
    _direct_native_anthropic_tool_cache_capability = _forward("hermes_core.agent.agent_runtime_helpers", "_direct_native_anthropic_tool_cache_capability")
    ...
    def _provider_model_requires_responses_api(model: str, *, provider: Optional[str] = None) -> bool:
        ...
        if normalized_provider == "copilot":
            try:
                from hermes_core.runtime.models import _should_use_copilot_responses_api
    ...
    _strip_think_blocks = _forward("hermes_core.agent.agent_runtime_helpers", "strip_think_blocks")
    ...
    _looks_like_codex_intermediate_ack = _forward("hermes_core.agent.agent_runtime_helpers", "looks_like_codex_intermediate_ack")
    _extract_reasoning = _forward("hermes_core.agent.agent_runtime_helpers", "extract_reasoning")
    _cleanup_task_resources = _forward("hermes_core.agent.chat_completion_helpers", "cleanup_task_resources")

    # Background memory/skill review — prompts live in agent.background_review.
    from hermes_core.agent.background_review import _MEMORY_REVIEW_PROMPT, _SKILL_REVIEW_PROMPT, _COMBINED_REVIEW_PROMPT
    _summarize_background_review_actions = _forward_static("hermes_core.agent.background_review", "summarize_background_review_actions")
    ...
    def _spawn_background_review(self, messages_snapshot: List[Dict], review_memory: bool = False,
        ...
        task_cfg = None
        if focus is None:
            from hermes_core.agent.background_review import load_background_review_settings
        ...
        from hermes_core.agent.turn_finalizer import _clone_background_review_messages
        ...
        if focus is None and not explicit and _review_should_defer(self, task_cfg):
            from hermes_core.agent.review_idle_queue import QUEUE
    ...
    def _spawn_background_review_now(self, messages_snapshot: List[Dict], review_memory: bool = False,
        ...
        """
        from hermes_core.agent.background_review import (
            finish_background_review_run, prepare_background_review_run, spawn_background_review_thread,
        )
        from hermes_core.tools.thread_context import propagate_context_to_thread
    ...
    _build_memory_write_metadata = _forward("hermes_core.agent.background_review", "build_memory_write_metadata")
    _apply_pending_steer_to_tool_results = _forward("hermes_core.agent.agent_runtime_helpers", "apply_pending_steer_to_tool_results")

    def get_activity_summary(self) -> dict:
        """Diagnostic snapshot: ``last_activity_*`` plus the short aliases gateway and delegate readers use."""
        from hermes_core.agent.session_activity import build_activity_snapshot
    ...
    def _close_task_resources(self, task_id: str) -> None:
        ...
        def kill_processes() -> None:
            from hermes_core.tools.process_registry import process_registry
            process_registry.kill_all(task_id=task_id)

        def release_computer_use() -> None:
            from hermes_core.tools.computer_use.tool import release_computer_use_session
    ...
    def _trim_process_memory() -> None:
        ...
        from hermes_core.runtime.mem_trim import trim_memory
    ...
    def _finalize_owned_session_row(self) -> None:
        ...
        if getattr(self, "_owns_session_db", False) and session_db is not None:
            ...
            # See #90837.
            from hermes_core.seams.session_state import release_or_close
    ...
    def _latest_todo_response(self, history: List[Dict[str, Any]]) -> Optional[tuple]:
        ...
        from hermes_core.tools.todo_tool import MAX_TODO_RESULT_CHARS
    ...
    _build_system_prompt = _forward("hermes_core.agent.system_prompt", "build_system_prompt")
    ...
    _sanitize_api_messages = _forward_static("hermes_core.agent.agent_runtime_helpers", "sanitize_api_messages")
    ...
    def _is_thinking_only_assistant(msg: Dict[str, Any], *, drop_codex_reasoning_items: bool = True) -> bool:
        ...
        # #82108.
        from hermes_core.agent.native_compaction import has_compaction_checkpoint
    ...
    _drop_thinking_only_and_merge_users = _forward_static("hermes_core.agent.agent_runtime_helpers", "drop_thinking_only_and_merge_users")
    ...
    _repair_tool_call = _forward("hermes_core.agent.agent_runtime_helpers", "repair_tool_call")
    _invalidate_system_prompt = _forward("hermes_core.agent.system_prompt", "invalidate_system_prompt")
    ...
    _interruptible_api_call = _forward("hermes_core.agent.chat_completion_helpers", "interruptible_api_call")
    _interruptible_streaming_api_call = _forward("hermes_core.agent.chat_completion_helpers", "interruptible_streaming_api_call")
    _try_activate_fallback = _forward("hermes_core.agent.chat_completion_helpers", "try_activate_fallback")
    ...
    _restore_primary_runtime = _forward("hermes_core.agent.agent_runtime_helpers", "restore_primary_runtime")
    _try_recover_primary_transport = _forward("hermes_core.agent.agent_runtime_helpers", "try_recover_primary_transport")
    _build_api_kwargs = _forward("hermes_core.agent.chat_completion_helpers", "build_api_kwargs")
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        try:
            ...
            from hermes_core.agent.tool_dispatch_helpers import _plan_tool_batch_segments
            ...
            if len(segments) == 1:
                ...
                return run(*args)
            from hermes_core.agent.tool_executor import execute_tool_calls_segmented
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        from hermes_core.tools.delegate_tool import _strip_model_hidden_task_fields, delegate_task as _delegate_task
    ...
    _invoke_tool = _forward("hermes_core.agent.agent_runtime_helpers", "invoke_tool")
    ...
    _execute_tool_calls_concurrent = _forward("hermes_core.agent.tool_executor", "execute_tool_calls_concurrent")
    _execute_tool_calls_sequential = _forward("hermes_core.agent.tool_executor", "execute_tool_calls_sequential")
    _handle_max_iterations = _forward("hermes_core.agent.chat_completion_helpers", "handle_max_iterations")
...
def _print_tool_listing() -> None:
    ...
    from hermes_core.model_tools import get_all_tool_names, get_available_toolsets
    from hermes_core.toolsets import get_all_toolsets, get_toolset_info
...
_PLUGIN_COMPAT_LAZY = {
    'COMPRESSED_SUMMARY_METADATA_KEY': ('hermes_core.agent.context_compressor', 'COMPRESSED_SUMMARY_METADATA_KEY'),
    'ContextCompressor': ('hermes_core.agent.context_compressor', 'ContextCompressor'),
    'DEFAULT_AGENT_IDENTITY': ('hermes_core.agent.prompt_builder', 'DEFAULT_AGENT_IDENTITY'),
    'FailoverReason': ('hermes_core.agent.error_classifier', 'FailoverReason'),
    'OpenAI': ('hermes_core.agent.process_bootstrap', 'OpenAI'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'build_context_files_prompt': ('hermes_core.agent.prompt_builder', 'build_context_files_prompt'),
    'build_environment_hints': ('hermes_core.agent.prompt_builder', 'build_environment_hints'),
    'build_skills_system_prompt': ('hermes_core.agent.prompt_builder', 'build_skills_system_prompt'),
    'check_toolset_requirements': ('model_tools', 'check_toolset_requirements'),
    'convert_scratchpad_to_think': ('hermes_core.agent.trajectory', 'convert_scratchpad_to_think'),
    'estimate_request_tokens_rough': ('hermes_core.agent.model_metadata', 'estimate_request_tokens_rough'),
    'file_mutation_result_landed': ('hermes_core.agent.tool_result_classification', 'file_mutation_result_landed'),
    'flatten_message_text': ('hermes_core.agent.message_content', 'flatten_message_text'),
    ...
    'jittered_backoff': ('hermes_core.agent.retry_utils', 'jittered_backoff'),
    'load_soul_md': ('hermes_core.agent.prompt_builder', 'load_soul_md'),
    'normalize_usage': ('hermes_core.agent.usage_pricing', 'normalize_usage'),
    'redact_sensitive_text': ('hermes_core.agent.redact', 'redact_sensitive_text'),
    'request_hard_interrupt': ('hermes_core.agent.interrupt_compat', 'request_hard_interrupt'),
    'sanitize_context': ('hermes_core.agent.memory_manager', 'sanitize_context'),
    'user_originated_turn_view': ('hermes_core.agent.context_compressor', 'user_originated_turn_view'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from hermes_core.runtime.plugin_compat import warn_once
