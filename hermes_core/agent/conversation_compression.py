...
from hermes_core.seams.auxiliary_client import AuxiliaryExplicitCancellation
from hermes_core.agent.context_engine import automatic_compaction_status_message, sanitize_memory_context
from hermes_core.agent.memory_provider import PRE_COMPRESS_CHECKPOINT_API_VERSION
from hermes_core.agent.model_metadata import estimate_messages_tokens_rough, estimate_request_tokens_rough
from hermes_core.agent.session_activity import ActivityProvenance, normalize_activity_provenance
...
def _strip_marker_for_comparison(msgs: Any) -> Any:
    ...
    from hermes_core.agent.context_compressor import _DB_PERSISTED_MARKER
...
def _refresh_agent_tool_definitions(agent) -> bool:
    ...
    from hermes_core.tools.mcp_tool_agent import refresh_agent_mcp_tools
...
def _get_compress_timeout_executor():
    ...
    if executor is not None:
        return executor
    from hermes_core.tools.daemon_pool import DaemonThreadPoolExecutor
...
def resolve_compression_fallback_route() -> Optional[dict]:
    ...
    try:
        from hermes_core.seams.auxiliary_client import _fallback_entry_api_key, _get_auxiliary_task_config
    ...
    for index, entry in enumerate(chain):
        ...
        try:
            ...
            api_key = None
        from hermes_core.seams.auxiliary_client import _coerce_positive_timeout
...
def _retry_compression_on_fallback_chain(
    *, worker: Callable[[CompressionCommitFence], Tuple[list, str]], messages: list,
    ...
    try:
        from hermes_core.agent.context_compressor import pin_summary_route
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    from hermes_core.agent.context_compressor import COMPRESSED_SUMMARY_METADATA_KEY
...
def _lower_threshold_to_aux_context(
    ...
    from hermes_core.agent.context_compressor import ContextCompressor as _CC
...
def check_compression_model_feasibility(agent: Any) -> None:
    ...
    try:
        ...
        from hermes_core.agent.model_metadata import MINIMUM_CONTEXT_LENGTH, get_model_context_length
...
def _steer_markers() -> Tuple[str, str]:
    ...
    try:
        from hermes_core.agent.prompt_builder import STEER_MARKER_CLOSE, STEER_MARKER_OPEN
...
def _strip_stale_todo_snapshot(content: Any) -> Any:
    ...
    from hermes_core.tools.todo_tool import TODO_INJECTION_HEADER
    if isinstance(content, str):
...
def _pruned_skill_reload_notice(compressed: list) -> str:
    ...
    from hermes_core.agent.context_compressor import _MAX_PRUNED_SKILL_MARKERS, _extract_pruned_skill_names
    names: list = []
...
def _insert_real_user_anchor(messages: list, anchor: dict) -> CompressedUserTurnOutcome:
    ...
    from hermes_core.agent.context_compressor import _DB_PERSISTED_MARKER, ContextCompressor
...
def _ensure_compressed_has_user_turn(original_messages: list, compressed: list) -> CompressedUserTurnOutcome:
    ...
    from hermes_core.agent.context_compressor import (
...
def _run_summary_dispatch(
    ...
    from hermes_core.seams.auxiliary_client import aux_interrupt_protection, aux_progress_hook, aux_stream_deadline
...
def _fold_todo_snapshot(agent: Any, compressed: list) -> None:
    ...
    if todo_snapshot:
        ...
        from hermes_core.agent.context_compressor import _append_text_to_content
        merged = False
...
def _salvage_or_refuse_grown_transcript(
    ...
    if _rough_out > _rough_in:
        ...
        from hermes_core.agent.context_compressor import salvage_grown_transcript
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from hermes_core.seams.session_state import is_automatic_end_reason
...
def _carry_session_state_to_child(agent: Any, old_session_id: str, old_title: Any) -> None:
    ...
    with _swallow('Could not migrate goal on compression: %s'):
        ...
        from hermes_core.runtime.goals import migrate_goal_to_session
        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
    with _swallow('Could not migrate heartbeat on compression: %s'):
        from hermes_core.runtime.heartbeat import migrate_heartbeat_to_session
        migrate_heartbeat_to_session(old_session_id, agent.session_id)
    with _swallow('Could not migrate loop on compression: %s'):
        from hermes_core.runtime.loops import migrate_loop_to_session
...
def _reset_read_dedup_caches(task_id: str, *, skills: bool = True) -> None:
    ...
    """
    with contextlib.suppress(Exception):
        from hermes_core.tools.file_tools_read_tracking import reset_file_dedup
        reset_file_dedup(task_id)
    if not skills:
        return
    with contextlib.suppress(Exception):
        from hermes_core.tools.skills_tool import reset_skill_view_dedup
    ...
    if agent._session_db:
        ...
        try:
            ...
            if in_place:
                ...
                from hermes_core.agent.context_compressor import PROACTIVE_PRUNE_REARM_MODEL_CONFIG_KEY, stamp_db_persisted_markers
...
def _record_codex_compaction_failure(agent: Any, error: str) -> None:
    ...
    from hermes_core.agent.context_compressor import _SUMMARY_FAILURE_COOLDOWN_SECONDS
    compressor = getattr(agent, "context_compressor", None)
...
def _compress_context_via_codex_app_server(
    ...
    with _swallow('codex compaction bookkeeping failed', exc_info=True):
        from hermes_core.agent.codex_runtime import _record_codex_app_server_compaction, _record_codex_app_server_usage
...
def try_shrink_image_parts_in_messages(api_messages: list, *, max_dimension: int = 8000) -> bool:
    ...
    if not api_messages:
        return False
    try:
        from hermes_core.tools.vision_tools import _resize_image_for_vision
    except Exception as exc:
