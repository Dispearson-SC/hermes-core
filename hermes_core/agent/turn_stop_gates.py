...
def _verify_on_stop_nudge(agent) -> Optional[str]:
    try:
        from hermes_core.agent.verification_stop import (
...
def _pre_verify_nudge(agent, final_response, attempt: int) -> Optional[str]:
    ...
    try:
        from hermes_core.agent.verify_hooks import max_verify_nudges
        from hermes_core.runtime.lifecycle import has_hook
        from hermes_core.runtime.plugins import get_pre_verify_continue_message

        if _edited and has_hook("pre_verify") and attempt < max_verify_nudges():
            # Posture is fixed for the session — resolve once + cache.
            coding = getattr(agent, "_resolved_is_coding", None)
            if coding is None:
                from hermes_core.agent.coding_context import is_coding_context
...
def _kanban_stop_nudge(agent, messages) -> Optional[str]:
    ...
    try:
        from hermes_core.agent.kanban_stop import build_kanban_stop_nudge
