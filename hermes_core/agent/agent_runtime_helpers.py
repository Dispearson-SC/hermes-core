...
from hermes_core.agent.prompt_builder import format_steer_marker
from hermes_core.agent.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from hermes_core.agent.trajectory import convert_scratchpad_to_think
from hermes_core.seams.credential_pool import (
    STATUS_EXHAUSTED, credential_pool_matches_provider, resolve_runtime_pool_key
)
from hermes_core.agent.error_classifier import FailoverReason
from hermes_core.agent.turn_context import drop_stale_api_content
from hermes_core.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
...
def _merge_consecutive_users(messages: List[Dict]) -> Tuple[List[Dict], int]:
    ...
    from hermes_core.agent.context_compressor import split_user_originated_turn
    ...
    def _rotate_and_swap(default_status: int, label: str) -> bool:
        ...
        if effective_reason is not None:
            ...
            if effective_reason == FailoverReason.billing and billing_unverified:
                ...
                from hermes_core.seams.credential_pool import FAILURE_REASON_BILLING_UNVERIFIED
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        from hermes_core.agent.chat_completion_helpers import _reset_stale_streak, rewrite_prompt_model_identity
...
def dump_api_request_debug(
    ...
    try:
        ...
        from hermes_core.agent.session_persistence import _safe_session_filename_component
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from hermes_core.seams.config import load_config as _load_moa_cfg
        from hermes_core.runtime.moa_config import resolve_moa_preset
        from hermes_core.runtime.runtime_provider import resolve_runtime_provider
...
def _route_may_be_custom(agent, eff_provider: str, provider_lower: str, eff_base_url: str) -> bool:
    ...
    if custom_providers:
        ...
        from hermes_core.runtime.providers import custom_provider_aliases
    ...
    try:
        from hermes_core.runtime.providers import get_provider
...
def anthropic_prompt_cache_policy(
    ...
    from hermes_core.agent.anthropic_endpoints import _model_name_is_kimi_family
    ...
    if supports_cache_markers and (
        ...
        try:
            from hermes_core.seams.config import get_custom_provider_model_capability
    ...
    if is_anthropic_wire and is_minimax_route:
        from hermes_core.agent.model_metadata import _model_name_suggests_minimax_m3
    ...
    from hermes_core.agent.prompt_caching import ALIBABA_FAMILY_PROVIDERS, is_qwen_model
...
def _profile_for_base_url(base_url: str) -> Any | None:
    ...
    try:
        from hermes_core.providers import list_providers
...
def _ensure_copilot_headers(client_kwargs: dict) -> None:
    ...
    try:
        if base_url_host_matches(str(client_kwargs.get("base_url", "")), "githubcopilot.com"):
            from hermes_core.runtime.models import copilot_default_headers
...
def _gemini_native_client(agent, client_kwargs: dict, httpx_verify, *, reason: str, shared: bool):
    ...
    from hermes_core.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from hermes_core.seams.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    from hermes_core.agent.ssl_verify import resolve_httpx_verify
    ...
    if agent.provider == "opencode-free":
        from hermes_core.runtime.models import opencode_zen_free_headers
    ...
    from hermes_core.agent.codex_headers import apply_required_codex_headers
    ...
    from hermes_core.agent import process_bootstrap
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    from hermes_core.agent.agent_init import _custom_provider_extra_body_for_agent
    # Prefer the init-time cache (agent._custom_providers); reload only if absent.
    custom_providers = getattr(agent, "_custom_providers", None)
    if custom_providers is None:
        try:
            from hermes_core.seams.config import load_config, get_compatible_custom_providers
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from hermes_core.runtime.providers import determine_api_mode
    from hermes_core.agent.native_compaction import resolve_native_compaction_capabilities
    from hermes_core.runtime.models import opencode_provider_family
...
def _update_switch_compressor(agent, custom_providers, effective_context_length, snapshot) -> None:
    ...
    if custom_providers is None:
        try:
            from hermes_core.seams.config import get_compatible_custom_providers, load_config
            custom_providers = get_compatible_custom_providers(load_config())
        except Exception:
            custom_providers = None
...
def switch_model(
    ...
    try:
        ...
        from hermes_core.seams.config import load_config as _sm_load_config
    ...
    from hermes_core.agent.chat_completion_helpers import _reset_stale_streak
    _reset_stale_streak(agent)
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from hermes_core.runtime.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    from hermes_core.agent.inline_tool_executors import (
    ...
    try:
        from hermes_core.runtime.middleware import apply_tool_request_middleware
    ...
    from hermes_core.runtime.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from hermes_core.hermes_logging import _session_context
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    from hermes_core.agent.message_sanitization import apply_reasoning_content_policy
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral, Cerebras, Groq, SambaNova, …):
    ...
    """
    from hermes_core.agent.message_sanitization import reapply_reasoning_echo
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from hermes_core.agent.process_bootstrap import HERMES_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
