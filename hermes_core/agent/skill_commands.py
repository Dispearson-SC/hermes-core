...
from hermes_core.agent.prompt_cache_boundary import register_stable_prefix
from hermes_core.agent.skill_preprocessing import load_skills_config as _load_skills_config, preprocess_skill_content
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    if not raw_identifier:
        return None
    try:
        from hermes_core.tools.skills_tool import _skills_dir, skill_view
        from hermes_core.agent.skill_utils import normalize_skill_lookup_name
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    try:
        from hermes_core.agent.skill_utils import extract_skill_config_vars, parse_frontmatter, resolve_skill_config_values
...
def _build_skill_message(
    ...
    from hermes_core.tools.skills_tool import _skills_dir
...
def _render_skill_block(
    ...
    try:
        ...
        # Track active usage for Curator lifecycle management (#17782)
        # Track active usage for Curator lifecycle management (#17782)
        from hermes_core.tools.skill_usage import bump_use
...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]], resolve_command) -> None:
    ...
    from hermes_core.tools.skills_tool import _parse_frontmatter, skill_matches_platform, skill_matches_environment
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        from hermes_core.tools.skills_tool import _skills_dir, _get_disabled_skill_names
...
def _disabled_skill_names(platform: str | None = None) -> set:
    ...
    try:
        from hermes_core.agent.skill_utils import get_disabled_skill_names
