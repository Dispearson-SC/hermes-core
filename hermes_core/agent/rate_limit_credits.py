...
class RateLimitCreditsMixin:
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from hermes_core.agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        try:
            from hermes_core.agent.credits_tracker import dev_fixture_credits_state
        ...
        try:
            from hermes_core.agent.credits_tracker import parse_credits_headers
    ...
    def _emit_credits_notices(self) -> None:
        ...
        if not self._credits_notices_enabled() or state is None:
            return
        try:
            from hermes_core.agent.credits_tracker import evaluate_credits_notices, is_free_tier_model, new_credits_latch
            latch = getattr(self, "_credits_latch", None)
            if latch is None:
