...
def emit_terminal_post_tool_call(
    ...
    *,
    ...
    try:
        from hermes_core.model_tools import _emit_post_tool_call_hook
...
def _session_search(agent, args: dict, ctx: InlineToolContext) -> Any:
    ...
    if not session_db:
        from hermes_core.seams.session_state import format_session_db_unavailable
...
def _desktop_preview(agent, args: dict, ctx: InlineToolContext) -> Any:
    ...
    from hermes_core.tools.preview_tool import _handle_preview
...
INLINE_TOOL_EXECUTORS: Dict[str, InlineToolExecutor] = {
    "todo_list": _tool(
        "hermes_core.tools.todo_tool", "todo_tool", ("todos", "todos"), ("merge", "merge", False),
