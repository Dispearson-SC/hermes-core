...
from hermes_core.agent.lazy_forward import forward as _forward, forward_static as _forward_static, lazy_attr as _lazy_attr
from hermes_core.runtime.timeouts import get_provider_request_timeout
from hermes_core.utils import base_url_host_matches, env_float
...
_ROUTE_DEFAULT_HEADERS = (
    ("openrouter.ai", lambda self, url: _lazy_attr("hermes_core.seams.auxiliary_client", "build_or_headers")()),
    ("ai-gateway.vercel.sh", lambda self, url: dict(_lazy_attr("hermes_core.seams.auxiliary_client", "_AI_GATEWAY_HEADERS"))),
    ("integrate.api.nvidia.com", lambda self, url: _lazy_attr("hermes_core.seams.auxiliary_client", "build_nvidia_nim_headers")(url)),
    ("api.routermint.com", lambda self, url: _routermint_headers()),
    ("githubcopilot.com", lambda self, url: _lazy_attr("hermes_core.runtime.models", "copilot_default_headers")()),
    ("api.kimi.com", lambda self, url: dict(_lazy_attr("hermes_core.seams.auxiliary_client", "_AI_GATEWAY_HEADERS"))),
...
class ClientLifecycleMixin:
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        from hermes_core.agent.process_bootstrap import build_keepalive_http_client
        return build_keepalive_http_client(base_url, verify=verify)

    _create_openai_client = _forward("hermes_core.agent.agent_runtime_helpers", "create_openai_client")
    _force_close_tcp_sockets = _forward_static("hermes_core.agent.agent_runtime_helpers", "force_close_tcp_sockets")
    _cleanup_dead_connections = _forward("hermes_core.agent.agent_runtime_helpers", "cleanup_dead_connections")
    _run_codex_stream = _forward("hermes_core.agent.codex_runtime", "run_codex_stream")
    _recover_with_credential_pool = _forward("hermes_core.agent.agent_runtime_helpers", "recover_with_credential_pool")
    ...
    def _anthropic_oauth_flag(self, token: str) -> bool:
        ...
        from hermes_core.agent.anthropic_credentials import _is_oauth_token
        return _is_oauth_token(token) if self.provider == "anthropic" else False

    def _build_anthropic_client_for_key(self, key: tuple) -> Any:
        from hermes_core.agent.anthropic_adapter import build_anthropic_bedrock_client, build_anthropic_client
        if key[0] == "bedrock":
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from hermes_core.runtime import auth as _auth
            resolve = (
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        if self.provider != "nous" or self.api_mode not in ("chat_completions", "anthropic_messages"):
            return False
        try:
            from hermes_core.runtime.auth import resolve_nous_runtime_credentials
    ...
    def _resolve_env_credentials(self) -> Optional[tuple]:
        ...
        """
        try:
            from hermes_core.seams.credential_pool import get_env_prefer_dotenv
        ...
        if pconfig and getattr(pconfig, "auth_type", "") == "api_key" and getattr(pconfig, "api_key_env_vars", ()):
            ...
            try:
                from hermes_core.runtime.runtime_provider import _get_named_custom_provider
            except ImportError:
                return None
    ...
    def _try_refresh_vertex_client_credentials(self) -> bool:
        ...
        if self.api_mode != "chat_completions" or self.provider != "vertex":
            return False
        try:
            from hermes_core.agent.vertex_adapter import get_vertex_config
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from hermes_core.runtime.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from hermes_core.seams.config import apply_custom_provider_extra_headers_to_client_kwargs
    ...
    def _apply_user_default_headers(self) -> None:
        ...
        if self.api_mode in ("anthropic_messages", "bedrock_converse"):
            return
        from hermes_core.seams.auxiliary_client import _apply_user_default_headers as _merge_user_headers
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        from hermes_core.agent.anthropic_adapter import create_anthropic_message
