...
from hermes_core.seams.paths import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("HERMES_KANBAN_TASK") or os.getenv("HERMES_KANBAN_BOARD"):
        ...
        if owned:
            return True
    try:
        from hermes_core.tools.kanban_tools import _profile_has_kanban_toolset
...
def _detect_docker() -> bool:
    try:
        from hermes_core.seams.paths import is_container
        return is_container()
    except Exception:
        return False
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        from hermes_core.tools.skills_guard import scan_skill_cached
        from hermes_core.seams.paths import get_hermes_home
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    try:
        # See #67277.
        from hermes_core.tools import skills_tool as _skills_tool
