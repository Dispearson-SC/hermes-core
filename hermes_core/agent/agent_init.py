...
from hermes_core.agent.context_compressor import ContextCompressor
from hermes_core.agent.agent_runtime_helpers import _ra
from hermes_core.agent.iteration_budget import IterationBudget
from hermes_core.agent.memory_manager import StreamingContextScrubber
from hermes_core.agent.session_activity import ActivityProvenance
from hermes_core.agent.model_metadata import (
...
from hermes_core.agent.subdirectory_hints import SubdirectoryHintTracker
from hermes_core.agent.think_scrubber import StreamingThinkScrubber
from hermes_core.agent.tool_guardrails import (
    ToolCallGuardrailConfig, ToolCallGuardrailController
)
from hermes_core.seams.config import cfg_get
from hermes_core.runtime.route_identity import normalize_route_base_url
from hermes_core.runtime.timeouts import get_provider_request_timeout
from hermes_core.seams.paths import get_hermes_home
from hermes_core.utils import base_url_host_matches, is_truthy_value
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from hermes_core.runtime.providers import HERMES_OVERLAYS, get_provider
    ...
    with suppress(Exception):
        from hermes_core.providers import get_provider_profile
        add(getattr(get_provider_profile(provider), "base_url", ""))

    with suppress(Exception):
        from hermes_core.runtime.auth import PROVIDER_REGISTRY
        from hermes_core.runtime.models import normalize_provider as normalize_model_provider
        from hermes_core.runtime.providers import normalize_provider as normalize_registry_provider
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if api_mode in _EXPLICIT_API_MODES:
        ...
        from hermes_core.runtime.providers import nous_api_mode
        ...
        try:
            ...
            from hermes_core.runtime.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    if credential_pool is not None:
        try:
            from hermes_core.seams.credential_pool import credential_pool_matches_provider
    ...
    with suppress(Exception):
        from hermes_core.runtime.model_normalize import (
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from hermes_core.seams.config import load_config_readonly as _load_pc_cfg
        from hermes_core.agent.agent_runtime_helpers import cache_ttl_means_disabled
...
def _init_turn_state(agent, run_budget_seconds):
    ...
    from hermes_core.agent.credits_tracker import new_credits_latch
...
def _setup_logging(agent):
    ...
    from hermes_core.hermes_logging import setup_logging, setup_verbose_logging
...
def _print_key_banner(key, label: str, warn_missing: bool = False) -> None:
    ...
    from hermes_core.agent.azure_identity_adapter import is_token_provider
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    from hermes_core.agent.anthropic_adapter import build_anthropic_client
    from hermes_core.agent.anthropic_credentials import resolve_anthropic_token
    agent.client = None
    agent._client_kwargs = {}
    agent._anthropic_base_url = base_url
    if agent.provider == "bedrock":
        # AnthropicBedrock SDK for full feature parity (prompt caching, thinking budgets).
        from hermes_core.agent.anthropic_adapter import build_anthropic_bedrock_client
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from hermes_core.runtime.auth import build_minimax_oauth_token_provider
    ...
    from hermes_core.agent.anthropic_credentials import _is_oauth_token as _is_oat
...
def _init_moa_client(agent, api_key):
    ...
    from hermes_core.agent.moa_loop import build_moa_facade
...
def _init_bedrock_client(agent, base_url):
    ...
    with suppress(Exception):
        from hermes_core.seams.config import load_config_readonly as _load_br_cfg
...
def _explicit_client_kwargs(agent, api_key, base_url, _provider_timeout) -> Dict[str, Any]:
    ...
    if _headers_for is not None:
        ...
        with suppress(Exception):
            from hermes_core.providers import get_provider_profile as _gpf
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    """
    from hermes_core.seams.auxiliary_client import resolve_provider_client
    ...
    for _fb in _fallback_entries(fallback_model):
        try:
            from hermes_core.runtime.fallback_config import resolve_entry_api_key
...
def _init_openai_client(agent, api_key, base_url, fallback_model, _provider_timeout):
    ...
    try:
        from hermes_core.agent.bedrock_adapter import configure_bedrock_openai_client_kwargs
    ...
    try:
        from hermes_core.agent.ssl_guard import verify_ca_bundle
        verify_ca_bundle()
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ("openrouter.ai", _lazy_headers("hermes_core.seams.auxiliary_client", "build_or_headers")),
    ("integrate.api.nvidia.com",
     _lazy_headers("hermes_core.seams.auxiliary_client", "build_nvidia_nim_headers", pass_base=True)),
    ("api.routermint.com", _lazy_headers("hermes_core.agent.client_lifecycle", "_routermint_headers")),
    ("githubcopilot.com", _lazy_headers("hermes_core.runtime.models", "copilot_default_headers")),
...
def _init_fallback_chain(agent, fallback_model):
    ...
    from hermes_core.agent.agent_runtime_helpers import sync_credential_pool_entry_id
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    ...
    try:
        from hermes_core.runtime.plugins import discover_plugins
        discover_plugins()
    except Exception:
    ...
    try:
        from hermes_core.tools.registry import registry as _snapshot_registry
        agent._tool_snapshot_generation = _snapshot_registry._generation
    except Exception:
        agent._tool_snapshot_generation = 0
    import hermes_core.model_tools as model_tools
    ...
    from hermes_core.agent.prompt_builder import KANBAN_GUIDANCE
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        try:
            from hermes_core.agent.delegation_context import is_delegated_child_context
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    with suppress(Exception):
        from hermes_core.seams.config import load_config_readonly as _load_sess_cfg
    ...
    from hermes_core.tools.checkpoint_manager import CheckpointManager
    ...
    with suppress(Exception):
        from hermes_core.tools.approval import _YOLO_MODE_FROZEN
        if _YOLO_MODE_FROZEN:
            agent._session_init_model_config["yolo_mode"] = True

    # In-memory todo list for task planning (one per agent/session)
    from hermes_core.tools.todo_tool import TodoStore
...
def _init_memory(agent, _agent_cfg, skip_memory, platform):
    ...
    if not skip_memory or _memory_toolset_requested:
        ...
        with suppress(Exception):
            from hermes_core.tools.memory_tool import (
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from hermes_core.agent.memory_manager import MemoryManager as _MemoryManager
    ...
    from hermes_core.agent.memory_manager import inject_memory_provider_tools
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    from hermes_core.agent.empty_response_guard import resolve_guard_settings
    (
    ...
    if agent._environment_probe:
        with suppress(Exception):
            from hermes_core.tools.env_probe import warm_environment_probe_async
...
def _compression_threshold(agent, cfg: Dict[str, Any]) -> tuple[float, bool]:
    ...
    with suppress(Exception):
        from hermes_core.seams.auxiliary_client import (
...
def _custom_provider_configured_base_url(
    ...
    if isinstance(_user_providers, dict):
        from hermes_core.seams.config import is_provider_enabled
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from hermes_core.runtime.auth import resolve_provider as resolve_auth_provider
...
def _scope_context_length_to_default_runtime(
    ...
    if isinstance(_default, dict):
        from hermes_core.seams.config import split_model_config_default
    ...
    if _configured_default_model:
        with suppress(Exception):
            from hermes_core.runtime.model_normalize import normalize_model_for_provider
...
def _resolve_context_length(agent, _agent_cfg, base_url):
    ...
    try:
        from hermes_core.seams.config import get_compatible_custom_providers
    ...
    if _config_context_length is None and _custom_providers:
        with suppress(Exception):
            from hermes_core.seams.config import get_custom_provider_context_length
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from hermes_core.runtime.plugins import get_plugin_context_engine
...
def _compressor_max_tokens(agent):
    ...
    with suppress(Exception):
        from hermes_core.agent.gemini_native_adapter import (
...
def _build_context_engine(agent, _agent_cfg, cs, _custom_providers, _effective_context_length, session_db):
    ...
    if _selected_engine is not None:
        ...
        from hermes_core.agent.model_metadata import get_model_context_length
    ...
    from hermes_core.agent.native_compaction import resolve_native_compaction_capabilities
...
def _warn_nonagentic_hermes_model(agent):
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from hermes_core.runtime.model_switch import _check_hermes_model_warning
...
def _inject_context_engine_tools(agent):
    ...
    if (
        ...
        from hermes_core.agent.memory_manager import normalize_tool_schema
...
def _init_usage_state(agent):
    from hermes_core.agent.runtime_cwd import scope_terminal_cwd
...
def init_agent(
    ...
    try:
        from hermes_core.seams.config import load_config_readonly as _load_agent_config
...
_PLUGIN_COMPAT_LAZY = {
    'ToolGuardrailDecision': ('hermes_core.agent.tool_guardrails', 'ToolGuardrailDecision'),
}
