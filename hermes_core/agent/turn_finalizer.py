...
from hermes_core.agent.message_sanitization import _sanitize_surrogates
...
def _record_kanban_budget_exhausted(
    ...
    """
    try:
        from hermes_core.runtime import kanban_db as _kb
        from hermes_core.runtime import kanban_db_connect as _kbc
        from hermes_core.runtime import kanban_db_dispatch as _kbd
        _conn = _kbc.connect()
        try:
...
def _close_transcript_tail(agent, messages, final_response, interrupted, _recovered_from_stream) -> None:
    ...
    if interrupted:
        from hermes_core.agent.message_sanitization import close_interrupted_tool_sequence
...
def finalize_turn(
    ...
    from hermes_core.agent.conversation_loop import logger
    ...
    try:
        from hermes_core.agent.conversation_loop import _notify_context_engine_turn_complete
