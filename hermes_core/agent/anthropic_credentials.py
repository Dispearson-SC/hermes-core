...
def _fingerprint(secret: Any) -> Optional[str]:
    from hermes_core.agent.credential_persistence import fingerprint_secret_value
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from hermes_core.runtime.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    try:
        from hermes_core.seams.credential_pool import AUTH_TYPE_OAUTH, load_pool
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from hermes_core.runtime.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
