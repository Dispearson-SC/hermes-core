...
from hermes_core.agent.conversation_compression import COMPRESSION_RETRY_CONTEXT_REDUCED_STATUS_TEMPLATE
from hermes_core.agent.model_metadata import is_output_cap_error, parse_available_output_tokens_from_error
from hermes_core.agent.retry_utils import is_zai_coding_overload_error, zai_coding_overload_retry_ceiling
...
from hermes_core.agent.thinking_timeout_guidance import build_thinking_timeout_guidance, is_thinking_timeout
from hermes_core.agent.turn_retry_state import TurnRetryState
from hermes_core.utils import base_url_host_matches

logger = logging.getLogger("hermes_core.agent.conversation_loop")
...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    from hermes_core.agent.conversation_loop import _print_nous_entitlement_guidance
    from hermes_core.seams.paths import display_hermes_home
    _body_text = ""
    try:
...
def _refresh_credentials_after_401(
    ...
    from hermes_core.agent.conversation_loop import _is_copilot_provider
...
def _recover_format_errors(
    ...
    if (
        ...
        from hermes_core.agent.native_compaction import is_native_compaction_rejection
    ...
    if classified.reason == FailoverReason.llama_cpp_grammar_pattern and not _retry.llama_cpp_grammar_retry_attempted:
        ...
        try:
            from hermes_core.tools.schema_sanitizer import strip_pattern_and_format
...
def recover_after_classification(
    ...
    from hermes_core.agent.conversation_loop import _is_nous_inference_route
    ...
    if (
        ...
        try:
            from hermes_core.runtime.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    from hermes_core.agent.conversation_loop import _print_billing_or_entitlement_guidance, _print_nous_entitlement_guidance
...
def nonretryable_client_error_result(
    ...
    from hermes_core.agent.conversation_loop import (
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from hermes_core.runtime.model_normalize import suggest_prefixed_model_id
...
def compute_error_backoff(
    ...
    from hermes_core.agent.retry_utils import adaptive_rate_limit_backoff, jittered_backoff
...
def _is_genuine_nous_rate_limit(agent: Any, api_error: Exception, error_context: Any) -> bool:
    ...
    _genuine = False
    try:
        from hermes_core.agent.nous_rate_guard import is_genuine_nous_rate_limit, record_nous_rate_limit
...
def route_classified_error(
    ...
    from hermes_core.agent.conversation_compression import conversation_history_after_compression
    from hermes_core.agent.conversation_loop import _arm_fallback_restart, _ra
    from hermes_core.agent.model_metadata import estimate_request_tokens_rough
