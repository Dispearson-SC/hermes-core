...
from hermes_core.seams.paths import (
...
from hermes_core.tools.threat_patterns import scan_for_threats as _scan_for_threats
...
def _plugin_backend_attr(backend: str, attr: str, default=None):
    ...
    try:
        from hermes_core.agent.terminal_env_registry import provider_flag
...
def _tenv_read(name: str, default: str = "") -> str:
    ...
    try:
        from hermes_core.tools.terminal_scope import terminal_env
    except ImportError:
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    from hermes_core.tools.terminal_tool_backends import _container_config_from_config, _create_environment, _ssh_config_from_config
    from hermes_core.tools.terminal_tool_lifecycle import _cleanup_env
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    if formatted is None:
        formatted = ""
        try:
            import hermes_core.tools.terminal_tool as terminal_tool  # heavy; only needed for non-local backends
...
def build_skills_system_prompt(
    ...
    try:
        ...
        from hermes_core.agent.skill_utils import get_project_skills_dirs
...
def _build_skills_system_prompt_inner(
    ...
    if project_dirs:
        from hermes_core.agent.skill_utils import iter_project_skill_files
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    ...
    """
    try:
        from hermes_core.seams.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
...
def build_context_files_prompt(
    ...
    # in-tree). See #64590.
    from hermes_core.agent.runtime_cwd import _is_install_tree
...
_PLUGIN_COMPAT_LAZY = {
    'org_id_of_path': ('hermes_core.agent.skill_utils', 'org_id_of_path'),
}
