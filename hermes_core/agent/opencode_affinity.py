...
def is_opencode_target(provider: Optional[str], base_url: Optional[str]) -> bool:
    ...
    try:
        ...
        pass
    try:
        from hermes_core.agent.anthropic_endpoints import _is_opencode_endpoint
...
def opencode_session_headers(
    ...
    if not is_opencode_target(provider, base_url):
        return {}
    try:
        from hermes_core.agent.portal_tags import get_affinity_scope, get_conversation_context
        from hermes_core.agent.transports.codex import _cache_scope_from_session_id
