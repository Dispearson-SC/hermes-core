...
try:
    ...
    # ---------------------------------------------------------------------------
    from hermes_core.tools.lazy_deps import ensure
...
def build_converse_kwargs(
    ...
    from hermes_core.agent.anthropic_adapter import _forbids_sampling_params
...
def probe_bedrock_context_length(model_id: str, region: str) -> Optional[int]:
    ...
    from hermes_core.agent.model_metadata import parse_context_limit_from_error
    try:
        client = _get_bedrock_runtime_client(region)
