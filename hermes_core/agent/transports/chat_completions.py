...
from hermes_core.agent.moonshot_schema import is_moonshot_model, sanitize_moonshot_tools
from hermes_core.agent.prompt_builder import DEVELOPER_ROLE_MODELS
...
def _rename_tool_search_bridge_for_xai(tools: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    """
    from hermes_core.agent.transports.codex import _alias_reserved_tools
...
def _add_prompt_cache_key(
    ...
    from hermes_core.agent.transports.codex import (
...
def _raise_gemini_thinking_max_tokens(model: str, reasoning_config: dict | None, requested: Any) -> Any:
    ...
    if not thinking_config:
        return requested
    from hermes_core.agent.gemini_native_adapter import _effective_gemini_max_output_tokens
