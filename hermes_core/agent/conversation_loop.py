...
from hermes_core.agent.codex_responses_adapter import _summarize_user_message_for_log
from hermes_core.agent.fast_mode import begin_turn as begin_fast_mode_turn
from hermes_core.agent.message_metadata import append_message
from hermes_core.agent.message_sanitization import _repair_tool_call_arguments, _sanitize_surrogates
from hermes_core.agent.model_metadata import MINIMUM_CONTEXT_LENGTH, _estimate_tools_tokens_rough
from hermes_core.agent.process_bootstrap import _install_safe_stdio
from hermes_core.agent.prompt_caching import (
    build_prompt_cache_plan,
    effective_cache_ttl,
    strip_anthropic_cache_control,
    strip_anthropic_tool_cache_control,
)
from hermes_core.agent.runtime_cwd import resolve_agent_cwd
from hermes_core.agent.turn_context import PreflightCompressionTimedOut, build_turn_context
...
from hermes_core.agent.turn_api_call import handle_api_interrupt, nous_rate_limit_guard, perform_api_call
from hermes_core.agent.turn_api_error import handle_api_error
from hermes_core.agent.turn_api_request import build_api_request
from hermes_core.agent.turn_final_response import finish_text_response
from hermes_core.agent.turn_finalizer import finalize_turn
from hermes_core.agent.turn_iteration_prep import (
    announce_api_call,
    apply_retry_restarts,
    begin_iteration,
    prepare_iteration,
)
from hermes_core.agent.turn_loop_errors import handle_outer_loop_error
from hermes_core.agent.turn_preflight_gate import run_preflight_gate
from hermes_core.agent.turn_request_assembly import assemble_api_request
from hermes_core.agent.turn_response_check import check_api_response
from hermes_core.agent.turn_response_intake import normalize_model_response
from hermes_core.agent.turn_tool_round import run_tool_round
from hermes_core.hermes_logging import set_session_context
from hermes_core.tools.skill_provenance import set_current_write_origin
...
def _midturn_request_pressure_tokens(
    ...
    """
    try:
        from hermes_core.agent.codex_responses_adapter import estimate_native_responses_preflight_tokens
...
def _should_skip_model_call_for_reference_handoff(
    ...
    from hermes_core.agent.context_compressor import reference_handoff_would_drive_next_model_call
...
def _is_interpreter_shutdown_error(exc: Exception) -> bool:
    ...
    if isinstance(exc, RuntimeError):
        ...
        from hermes_core.tools.interpreter_shutdown import interpreter_shutting_down
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    if not (messages and messages[-1].get("role") == "assistant"):
        ...
        if not visible:
            ...
            from hermes_core.agent.agent_runtime_helpers import _INTERRUPTED_PLACEHOLDER
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if provider not in ("llamacpp", "llama.cpp", "llama-cpp", "custom") or not (
        ...
        return None
    try:
        from hermes_core.runtime.local_runtime.growth import maybe_grow_window
...
def _ra():
    ...
    import hermes_core.run_agent as run_agent
    return run_agent
...
def _billing_or_entitlement_message(
    *, capability: str, provider: str, base_url: str, model: str, unverified: bool = False
    ...
    try:
        from hermes_core.agent.billing_links import build_billing_block
...
def _bot_chat_prompt_stale(agent, stored_prompt: str) -> bool:
    ...
    re-fire."""
    try:
        from hermes_core.tools.bot_mode_probe import (
            BOT_CHAT_TITLE,
        ...
        home = None
        try:
            from hermes_core.agent.system_prompt import _agent_home
...
def _persist_system_prompt(agent, failure_message: str, *, persist_tools: bool = False) -> None:
    ...
    try:
        ...
        if persist_tools:
            from hermes_core.tools.mcp_tool_agent import persist_agent_tool_names
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        if _bot_chat_prompt_stale(agent, stored_prompt):
            ...
            try:
                from hermes_core.agent.prompt_builder import clear_skills_system_prompt_cache
        ...
        try:
            ...
            if saved_tools:
                from hermes_core.tools.mcp_tool_agent import restore_agent_tool_prefix
        ...
        from hermes_core.agent.system_prompt import reconstruct_static_prefix, restore_plugin_prompt_sections
    ...
    try:
        from hermes_core.runtime.lifecycle import invoke_hook as _invoke_hook
        _invoke_hook(
    ...
    try:
        from hermes_core.agent.credits_tracker import seed_credits_at_session_start
...
def _ensure_cached_system_prompt_static(agent, system_message=None) -> None:
    ...
    from hermes_core.agent.system_prompt import reconstruct_static_prefix
...
def _peel_moa_guidance(messages: List[Dict[str, Any]], guidance: Any) -> List[Dict[str, Any]]:
    ...
    from hermes_core.agent.moa_loop import peel_reference_guidance
...
def _redecorate_prompt_cache_for_provider(
    ...
    # AIAgent, and a default would mask a real init bug as silent cache-off.
        ...
        from hermes_core.agent.prompt_caching import envelope_tool_part_cache_markers_supported
...
def _engine_overrides_hook(engine: Any, name: str) -> bool:
    ...
    if engine is None or not callable(hook):
        return False
    try:
        from hermes_core.agent.context_engine import ContextEngine as _CE
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from hermes_core.runtime.moa_config import decode_moa_turn
...
def _preflight_timeout_result(agent, exc, conversation_history) -> Dict[str, Any]:
    ...
    from hermes_core.agent.agent_runtime_helpers import note_turn_persisted
...
_PLUGIN_COMPAT_LAZY = {
    'COMPRESSION_RETRY_CONTEXT_REDUCED_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'COMPRESSION_RETRY_CONTEXT_REDUCED_STATUS_TEMPLATE'),
    'COMPRESSION_RETRY_MESSAGES_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'COMPRESSION_RETRY_MESSAGES_STATUS_TEMPLATE'),
    'COMPRESSION_RETRY_TOKENS_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'COMPRESSION_RETRY_TOKENS_STATUS_TEMPLATE'),
    'COMPRESSION_RETRY_TOO_LARGE_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'COMPRESSION_RETRY_TOO_LARGE_STATUS_TEMPLATE'),
    'FailoverReason': ('hermes_core.agent.error_classifier', 'FailoverReason'),
    'KawaiiSpinner': ('hermes_core.agent.display', 'KawaiiSpinner'),
    'PARTIAL_STREAM_STUB_ID': ('hermes_constants', 'PARTIAL_STREAM_STUB_ID'),
    'PRE_API_COMPRESSION_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'PRE_API_COMPRESSION_STATUS_TEMPLATE'),
    'adaptive_rate_limit_backoff': ('hermes_core.agent.retry_utils', 'adaptive_rate_limit_backoff'),
    'anchored_context_tokens': ('hermes_core.agent.model_metadata', 'anchored_context_tokens'),
    'automatic_compaction_status_message': ('hermes_core.agent.context_engine', 'automatic_compaction_status_message'),
    'capture_usage_anchor': ('hermes_core.agent.model_metadata', 'capture_usage_anchor'),
    'classify_api_error': ('hermes_core.agent.error_classifier', 'classify_api_error'),
    'close_interrupted_tool_sequence': ('hermes_core.agent.message_sanitization', 'close_interrupted_tool_sequence'),
    'coalesce_tool_call_id': ('hermes_core.agent.message_sanitization', 'coalesce_tool_call_id'),
    'compose_user_api_content': ('hermes_core.agent.turn_context', 'compose_user_api_content'),
    'compression_blocked_transiently': ('hermes_core.agent.conversation_compression', 'compression_blocked_transiently'),
    'compression_skipped_due_to_lock': ('hermes_core.agent.conversation_compression', 'compression_skipped_due_to_lock'),
    'context_compression_timed_out': ('hermes_core.agent.conversation_compression', 'context_compression_timed_out'),
    'conversation_history_after_compression': ('hermes_core.agent.conversation_compression', 'conversation_history_after_compression'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'estimate_messages_tokens_rough': ('hermes_core.agent.model_metadata', 'estimate_messages_tokens_rough'),
    'estimate_request_tokens_rough': ('hermes_core.agent.model_metadata', 'estimate_request_tokens_rough'),
    'estimate_usage_cost': ('hermes_core.agent.usage_pricing', 'estimate_usage_cost'),
    'get_context_length_from_provider_error': ('hermes_core.agent.model_metadata', 'get_context_length_from_provider_error'),
    'has_incomplete_scratchpad': ('hermes_core.agent.trajectory', 'has_incomplete_scratchpad'),
    'is_output_cap_error': ('hermes_core.agent.model_metadata', 'is_output_cap_error'),
    'is_repetition_dominated': ('hermes_core.agent.repetition_guard', 'is_repetition_dominated'),
    'is_zai_coding_overload_error': ('hermes_core.agent.retry_utils', 'is_zai_coding_overload_error'),
    ...
    'parse_available_output_tokens_from_error': ('hermes_core.agent.model_metadata', 'parse_available_output_tokens_from_error'),
    'reanchor_current_turn_user_idx': ('hermes_core.agent.turn_context', 'reanchor_current_turn_user_idx'),
    'save_context_length': ('hermes_core.agent.model_metadata', 'save_context_length'),
    'serialized_messages_bytes': ('hermes_core.agent.message_sanitization', 'serialized_messages_bytes'),
    'splice_provider_projection': ('hermes_core.agent.provider_projection', 'splice_provider_projection'),
    'zai_coding_overload_retry_ceiling': ('hermes_core.agent.retry_utils', 'zai_coding_overload_retry_ceiling'),
}
