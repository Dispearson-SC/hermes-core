...
from hermes_core.agent.thread_scoped_output import thread_scoped_silence
...
def _interrupt_background_review(review_agent: Any) -> None:
    ...
    def _interrupt() -> None:
        try:
            from hermes_core.agent.interrupt_compat import request_hard_interrupt
...
def build_cache_parity_fork(
    ...
    teardown."""
    from hermes_core.run_agent import AIAgent  # local: avoids a circular import at load
...
def _set_thread_approval_callback(callback: Any) -> None:
    from hermes_core.tools.terminal_tool import set_approval_callback
...
def _review_tool_whitelist(review_agent: Any, task_cfg: Optional[Dict[str, Any]]) -> Tuple[set, set]:
    ...
    from hermes_core.model_tools import get_tool_definitions
...
def _run_review_fork(
    ...
    from hermes_core.runtime.plugins import set_thread_tool_whitelist, clear_thread_tool_whitelist
    ...
    with suppress(Exception):
        from hermes_core.tools.skill_manager_guards import _reset_background_review_read_marks
