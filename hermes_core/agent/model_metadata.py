...
from hermes_core.utils import atomic_json_write, atomic_yaml_write, base_url_host_matches, base_url_hostname

from hermes_core.seams.paths import OPENROUTER_MODELS_URL
from hermes_core.agent.message_metadata import PERSISTENCE_ONLY_MESSAGE_FIELDS
...
try:
    from hermes_core.providers import list_providers as _list_providers
except Exception:
    def _list_providers():
        return []
...
def _validate_cached_context_length(model: str, base_url: str, cached: int, is_bedrock_context: bool, *, api_key: str = "") -> Optional[int]:
    ...
    if is_bedrock_context:
        ...
        try:
            from hermes_core.agent.bedrock_adapter import get_bedrock_context_length
...
def _resolve_bedrock_context_length(model: str, base_url: str) -> Optional[int]:
    ...
    try:
        from hermes_core.agent.bedrock_adapter import get_bedrock_context_length, resolve_bedrock_region
    except ImportError:
...
def _config_override_context_length(model: str, base_url: str, provider: str, custom_providers: list | None) -> Optional[int]:
    ...
    if provider and model:
        with contextlib.suppress(Exception):  # fall through to other resolution paths
            from hermes_core.agent.models_dev import _override_context_window
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from hermes_core.runtime.models import get_copilot_model_context
    ...
    if effective_provider:
        from hermes_core.agent.models_dev import lookup_models_dev_context
