...
from hermes_core.agent.prompt_builder import (
...
from hermes_core.agent import prompt_builder as _pb
from hermes_core.agent.runtime_cwd import resolve_context_cwd
from hermes_core.seams.paths import get_default_hermes_root, get_hermes_home
from hermes_core.utils import is_truthy_value
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from hermes_core.runtime.plugins import render_system_prompt_sections
...
def _restore_plugin_prompt_sections(prompt: str) -> tuple:
    ...
    from hermes_core.runtime.plugins import (
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from hermes_core.runtime.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    """
    try:
        from hermes_core.seams.paths import get_hermes_home_override
...
def _skills_prompt(agent: Any) -> str:
    ...
    try:
        from hermes_core.agent.coding_context import coding_compact_skill_categories
...
def _bot_mode_parts(agent: Any) -> List[str]:
    ...
    parts: List[str] = []
    try:
        from hermes_core.tools.bot_mode_probe import BOT_CHAT_TITLE, epoch_line, get_bot_mode_protocol_section
...
def _ambient_file_safety_profile_name() -> str:
    from hermes_core.agent.file_safety import _resolve_active_profile_name
...
def _timestamp_line(agent: Any) -> str:
    ...
    from hermes_core.hermes_time import get_timezone as _hermes_tz, now as _hermes_now
    now = _hermes_now()
...
def _memory_parts(agent: Any) -> List[str]:
    ...
    if agent._memory_manager:
        try:
            from hermes_core.agent.memory_manager import memory_provider_tools_exposed as _mem_exposed
        except Exception:
            _mem_exposed = None
...
def _guidance_parts(agent: Any) -> List[str]:
    ...
    if _model_gate(getattr(agent, "_execution_guidance", "auto"), agent.model, EXECUTION_GUIDANCE_MODELS):
        from hermes_core.agent.prompt_builder import execution_guidance_text
...
def _coding_parts(agent: Any) -> Tuple[List[str], List[str], List[str]]:
    ...
    try:
        from hermes_core.agent.coding_context import coding_system_prompt_parts
        if agent.valid_tool_names:
...
def _post_workspace_parts(agent: Any) -> List[str]:
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from hermes_core.tools.env_probe import get_environment_probe_line
...
_PLUGIN_COMPAT_LAZY = {
    'OPENAI_MODEL_EXECUTION_GUIDANCE': ('hermes_core.agent.prompt_builder', 'OPENAI_MODEL_EXECUTION_GUIDANCE'),
}
