...
from hermes_core.agent.display import (
    KawaiiSpinner,
...
from hermes_core.agent.message_sanitization import coalesce_tool_call_id
...
from hermes_core.agent.tool_dispatch_helpers import (
...
from hermes_core.tools.tool_result_storage import (
    maybe_persist_tool_result,
    enforce_turn_budget,
    extract_persisted_path,
)
from hermes_core.tools.budget_config import BudgetConfig, DEFAULT_BUDGET, budget_for_context_window
...
def _ensure_file_checkpoint(agent, function_name: str, function_args: dict, effective_task_id: str) -> None:
    ...
    if not file_path:
        return
    from hermes_core.tools.file_tools_paths import _resolve_path_for_task
...
def _authorization_gate_lock_timeout() -> float:
    ...
    """
    try:
        from hermes_core.tools.approval_human_wait import human_wait_ceiling
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    from hermes_core.agent.deadline import resolve_timeout
...
def _flush_session_db_after_tool_progress(agent, messages: list, *, stage: str) -> bool:
    ...
    try:
        ...
        agent._incremental_persistence_failed = True
        from hermes_core.seams.session_state import classify_persistence_error
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    try:
        ...
        from hermes_core.tools import tool_search as _ts
        from hermes_core.tools.registry import registry as _registry
    except Exception:
        return frozenset()
...
def _canonical_tool_name(function_name: str) -> str:
    ...
    from hermes_core.model_tools import _LEGACY_TOOL_ALIASES as _lta
...
class _ConcurrentToolAuthorizationGate:
    ...
    def __init__(self, *, lock_timeout: float | None = None, session_key: str | None = None) -> None:
        ...
        if self._session_key is None:
            ...
            try:
                from hermes_core.tools.approval_context import get_current_session_key
    ...
    def _human_wait_seconds(self) -> float:
        try:
            from hermes_core.tools.approval_human_wait import human_wait_seconds
...
def _set_worker_activity_callback(agent) -> None:
    ...
    with contextlib.suppress(Exception):
        from hermes_core.tools.environments.base import set_activity_callback
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    from hermes_core.seams import relay_tools
    from hermes_core.runtime.middleware import (
