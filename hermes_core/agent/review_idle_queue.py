...
def review_targets_managed_local(agent: Any, task_cfg: Optional[Dict[str, Any]]) -> bool:
    ...
    try:
        from hermes_core.seams.auxiliary_client import _is_managed_local_endpoint, _managed_local_netloc

        if not _managed_local_netloc():
            return False
        from hermes_core.agent.background_review import _resolve_review_runtime
