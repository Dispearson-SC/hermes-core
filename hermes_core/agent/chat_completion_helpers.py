...
from hermes_core.seams.paths import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
from hermes_core.agent.error_classifier import (FailoverReason, PROVIDER_STREAM_NON_JSON_ERROR_CODE)
from hermes_core.agent.errors import EmptyStreamError
from hermes_core.agent.fast_mode import effective_request_overrides
from hermes_core.agent.turn_context import substitute_api_content
from hermes_core.agent.gemini_native_adapter import is_native_gemini_base_url
...
from hermes_core.agent.message_content import flatten_message_text
from hermes_core.agent.message_metadata import append_message, stamp_message_timestamp
from hermes_core.agent.message_sanitization import (_sanitize_surrogates, _repair_tool_call_arguments)
from hermes_core.agent.reasoning_summaries import separate_glued_reasoning_blocks
from hermes_core.agent.stream_single_writer import claim_stream_writer, stream_writer_is_current
...
from hermes_core.utils import base_url_host_matches, base_url_hostname, env_float, env_int
...
def _provider_stream_error_from_json_decode_error(error: json.JSONDecodeError, *,
    ...
    from hermes_core.agent.redact import redact_sensitive_text
...
def _prompt_cache_scope_for_agent(agent) -> "str | None":
    ...
    try:
        from hermes_core.agent.prompt_cache_scope import resolve_prompt_cache_scope_safe
...
def _bedrock_converse_call(api_kwargs: dict, *, stream: bool, on_stream_denied=None):
    ...
    from hermes_core.agent.bedrock_adapter import (_get_bedrock_runtime_client, invalidate_runtime_client,
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from hermes_core.runtime.local_runtime.load_progress import get_loading_progress, get_prefill_progress
        from hermes_core.runtime.local_runtime.supervisor import state_path
...
def _alias_tool_search_bridge_for_xai(agent, transport, tools_for_api):
    ...
    try:
        ...
        from hermes_core.agent.transports.chat_completions import _rename_tool_search_bridge_for_xai
        has_bridge = any(
...
def _build_codex_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    from hermes_core.agent.codex_responses_adapter import classify_responses_route
    from hermes_core.agent.native_compaction import native_compaction_context_management
    ...
    if is_xai_responses:
        try:
            ...
            from hermes_core.tools.schema_sanitizer import strip_pattern_and_format, strip_slash_enum
...
def _anthropic_max_output_for_model(agent):
    ...
    with contextlib.suppress(Exception):
        from hermes_core.agent.anthropic_adapter import _get_anthropic_max_output, _ANTHROPIC_OUTPUT_LIMITS
...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    with contextlib.suppress(Exception):
        from hermes_core.seams.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    """
    from hermes_core.agent.opencode_affinity import merge_opencode_session_headers
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    if (fb.get("provider") or "").strip().lower() != "nous":
        return None
    try:
        from hermes_core.runtime.auth import get_provider_auth_state
...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    from hermes_core.runtime.providers import host_mandated_api_mode
...
def _rebind_fallback_credential_pool(agent, fb_provider: str, fb_model: str) -> None:
    ...
    if getattr(agent, "_credential_pool", None) is None:
        try:
            from hermes_core.seams.credential_pool import load_pool
...
def _should_skip_fallback_candidate(agent, fb: dict, fb_key: tuple, fb_provider: str, fb_model: str, unavailable: set) -> bool:
    ...
    from hermes_core.agent.backend_identity import BackendIdentity, should_skip_candidate
...
def _reresolve_fallback_reasoning_config(agent) -> None:
    ...
    try:
        ...
        from hermes_core.seams.paths import resolve_reasoning_config
...
def _rescope_fallback_extra_body(agent, old_model: str, old_provider: str, old_base_url: str) -> None:
    ...
    try:
        from hermes_core.agent.agent_init import _custom_provider_extra_body_for_agent, _merge_custom_provider_extra_body
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if fb_base_url_hint and base_url_host_matches(fb_base_url_hint, "ollama.com") and not fb_api_key_hint:
            from hermes_core.agent.secret_scope import get_secret
...
def _iteration_summary_api_messages(agent, messages: list) -> list:
    ...
    from hermes_core.agent.context_compressor import evict_stale_outbound_tool_images
...
def _managed_summary_call(agent, api_request_id: str, request, callback, *, retry_count: int):
    from hermes_core.seams import relay_llm
...
def _iteration_summary_chat_kwargs(agent, api_messages: list) -> dict:
    ...
    try:
        from hermes_core.seams.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
    except Exception:
    ...
    if "nousresearch" in agent._base_url_lower:
        from hermes_core.agent.portal_tags import nous_portal_tags
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    from hermes_core.agent.context_compressor import MAX_ITERATIONS_SUMMARY_REQUEST
...
def cleanup_task_resources(agent, task_id: str) -> None:
    ...
    def _headed() -> bool:
        try:
            from hermes_core.tools.browser_tool_cloud import _is_headed_mode
...
class _BedrockStream:
    ...
    def _fall_back_to_converse(self, client, final_kwargs: dict, exc: Exception):
        ...
        from hermes_core.agent.bedrock_adapter import normalize_converse_response
    ...
    def _worker(self):
        ...
        try:
            from hermes_core.seams import relay_llm
            from hermes_core.agent.bedrock_adapter import stream_converse_with_callbacks
            intercepted_events = []
            ...
            try:
                from hermes_core.agent.plugin_stream_hooks import has_reasoning_stream_observer_hooks
    ...
    def _on_stale(self, stale_elapsed: float) -> None:
        ...
        try:
            from hermes_core.agent.bedrock_adapter import invalidate_runtime_client
...
class _StreamingCall:
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        self._writer_token = self._attempt_request_client = self._attempt_stream_response = None
        from hermes_core.agent.chat_completion_helpers_relay import RelayChatAccumulator
    ...
    def _call_anthropic(self, request_client):
        ...
        from hermes_core.seams import relay_llm
        from hermes_core.agent.anthropic_adapter import sanitize_anthropic_kwargs
    ...
    def _maybe_disable_streaming(self, e) -> None:
        ...
        if not _is_stream_unsupported and "invokemodelwithresponsestream" in _err_lower:
            ...
            from hermes_core.agent.bedrock_adapter import is_streaming_access_denied_error
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            ...
            with contextlib.suppress(Exception):
                from hermes_core.seams.config import load_config_readonly
    ...
    def _partial_stream_stub(self):
        ...
        with contextlib.suppress(Exception):
            from hermes_core.agent.error_classifier import classify_api_error
