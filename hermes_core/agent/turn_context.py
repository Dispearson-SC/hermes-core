...
from hermes_core.agent.conversation_compression import recover_rotated_compression_session
from hermes_core.agent.iteration_budget import IterationBudget
from hermes_core.agent.memory_manager import build_memory_context_block
...
def _agent_stale_thinking_on_wire(agent: Any) -> bool:
    ...
    try:
        from hermes_core.agent.message_sanitization import stale_thinking_reaches_wire
...
def _maybe_title_session_at_turn_start(agent: Any, messages: List[Any]) -> None:
    ...
    try:
        ...
        from hermes_core.agent.title_generator import maybe_auto_title
...
def reanchor_current_turn_user_idx(messages: List[Any], user_message: Any) -> int:
    ...
    """
    from hermes_core.agent.context_compressor import user_originated_turn_view
...
def _fail_closed_after_preflight_timeout(agent, request_tokens: int) -> None:
    ...
    from hermes_core.agent.conversation_compression import context_compression_timed_out
...
def _publish_runtime_main(agent: Any) -> None:
    ...
    with suppress(Exception):
        from hermes_core.seams.auxiliary_client import set_runtime_main
...
def _refresh_mcp_tools_between_turns(agent: Any) -> None:
    ...
    try:
        ...
        if not getattr(agent, "_skip_mcp_refresh", False) and "hermes_core.tools.mcp_tool" in sys.modules:
            from hermes_core.tools.mcp_tool_discovery import has_registered_mcp_tools
...
def _bind_turn_identity(
    ...
    from hermes_core.agent.agent_runtime_helpers import note_turn_start
...
def _emit_reaction(agent: Any, original_user_message: Any) -> None:
    ...
    if reaction_callback is None:
        return
    with suppress(Exception):
        from hermes_core.agent.reactions import detect_reaction
...
def _collect_pre_llm_call_context(
    ...
    try:
        ...
        try:
            ...
            from hermes_core.tools.hook_output_spill import (
...
def build_turn_context(
    ...
    compression."""
    from hermes_core.agent.turn_context_compaction import run_turn_start_compaction
    ...
    try:
        from hermes_core.tools.bot_mode_dm import ensure_message_agent_tool
...
def build_api_messages(
    ...
    replayed verbatim."""
    from hermes_core.agent.agent_runtime_helpers import fill_empty_non_final_wire_payload
    from hermes_core.agent.conversation_loop import _clone_message_for_send
...
_PLUGIN_COMPAT_LAZY = {
    'IDLE_COMPACTION_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'IDLE_COMPACTION_STATUS_TEMPLATE'),
    'PREFLIGHT_COMPRESSION_STATUS_TEMPLATE': ('hermes_core.agent.conversation_compression', 'PREFLIGHT_COMPRESSION_STATUS_TEMPLATE'),
