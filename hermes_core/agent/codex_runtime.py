...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    ...
    if not isinstance(usage, dict) or not usage:
        ...
        return {}
    from hermes_core.agent.usage_pricing import CanonicalUsage, estimate_usage_cost
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if not force:
        with suppress(Exception):
            from hermes_core.agent.conversation_compression import COMPACTION_STATUS
...
def _stable_call_id(item: dict, name: str) -> str:
    ...
    from hermes_core.agent.transports.codex_event_projector import _deterministic_call_id
    item_type = item.get("type") or ""
...
def _ensure_codex_session(agent) -> None:
    ...
    from hermes_core.agent.transports.codex_app_server_session import CodexAppServerSession, _ServerRequestRouting
    # Approval callback: Hermes' standard prompt flow when a CLI thread installed one.
    approval_callback = None
    with suppress(Exception):
        from hermes_core.tools.terminal_tool import _get_approval_callback
    ...
    try:
        from hermes_core.tools.approval import is_approval_bypass_active
...
def run_codex_app_server_turn(agent, *, user_message: str, original_user_message: Any, messages: List[Dict[str, Any]],
    ...
    if getattr(agent, "compression_checkpoint_required", False) is True:
        from hermes_core.agent.conversation_compression import _checkpoint_blocked
...
def _raise_stream_error(event: Any) -> None:
    ...
    from hermes_core.run_agent import _StreamErrorEvent
