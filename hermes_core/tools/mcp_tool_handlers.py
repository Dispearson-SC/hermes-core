...
from hermes_core.tools.registry import tool_error
from hermes_core.tools.ansi_strip import strip_unicode_tags
from hermes_core.tools.mcp_tool_common import _exc_str, _sanitize_error, mcp_field, _core
from hermes_core.tools import mcp_tool_loop as _loop
from hermes_core.tools.mcp_tool_content import (
...
from hermes_core.tools.mcp_tool_errors import _is_auth_error, _is_session_expired_error

logger = logging.getLogger("hermes_core.tools.mcp_tool")
_MISSING = object()
...
def _trust_gate_check(server_name: str, tool_name: str) -> Optional[str]:
    ...
    try:  # lazy: tools.approval routes the prompt to whichever surface owns the session
        from hermes_core.tools.approval_prompt import request_elicitation_consent
...
def _acquire_call_server(server_name: str, tool_timeout: float):
    ...
    from hermes_core.tools import mcp_tool_discovery as _discovery  # lazy: discovery -> registration -> handlers cycle
...
def _handle_auth_error_and_retry(server_name: str, exc: BaseException, retry_call, op_description: str):
    ...
    if not _is_auth_error(exc):
        return None
    from hermes_core.tools.mcp_oauth_manager import get_manager
    try:
...
def _make_utility_handler(op: str, log_label: str, rpc, render, required: Optional[str] = None):
    ...
    def _factory(server_name: str, tool_timeout: float):
        def _handler(args: dict, **kwargs) -> str:
            from hermes_core.tools import mcp_tool_discovery as _discovery  # lazy: import cycle
            server = _discovery._get_connected_server_for_call(server_name)
