...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    from hermes_core.tools import mcp_tool as _origin
    try:
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    try:
        from hermes_core.seams.paths import get_hermes_home_override, reset_hermes_home_override, set_hermes_home_override
...
def _wrap_with_dashboard_oauth_flow(coro):
    ...
    try:
        from hermes_core.tools.mcp_dashboard_oauth import dashboard_oauth_flow, get_dashboard_oauth_flow
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    from hermes_core.tools.interrupt import is_interrupted
    from hermes_core.agent.async_utils import safe_schedule_threadsafe
