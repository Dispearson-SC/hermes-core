...
from hermes_core.runtime._subprocess_compat import windows_hide_flags
...
def _uv_binary() -> Optional[str]:
    ...
    try:
        from hermes_core.runtime.managed_uv import resolve_uv
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    try:
        from hermes_core.tools.environments.local import hermes_subprocess_env
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    if _lazy_install_target() is None:
        ...
        with contextlib.suppress(Exception):
            from hermes_core.seams.config import get_managed_system
