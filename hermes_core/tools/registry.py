...
def _save_discovery_cache(cache: Dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from hermes_core.utils import atomic_json_write  # stdlib+yaml only; no cycle
...
class ToolRegistry:
    ...
    def dispatch(
        ...
        try:
            if entry.is_async:
                from hermes_core.model_tools import _run_async
            ...
            try:
                from hermes_core.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from hermes_core.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
