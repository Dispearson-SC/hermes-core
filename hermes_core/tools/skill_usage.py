...
from hermes_core.agent.skill_utils import is_excluded_skill_path, is_external_skill_path
from hermes_core.utils import atomic_write_text
...
def telemetry_provenance(skill_name: str, record: Optional[Dict[str, Any]] = None) -> str:
    ...
    if ":" in skill_name:
        with suppress(Exception):
            from hermes_core.runtime.plugins import get_plugin_manager
...
def _relocate(src: Path, dest: Path, skill_name: str, action: str, **capture_kwargs: Any) -> Tuple[bool, str]:
    ...
    try:
        from hermes_core.tools import skill_ledger as _ledger
...
def _find_external_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    from hermes_core.agent.skill_utils import get_all_skills_dirs
