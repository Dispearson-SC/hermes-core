...
# Removed during extraction -- atomic_roundtrip_yaml_update(): rewrites one key in the user's config.yaml, preserving comments -- this is `hermes config set`, a CLI concern. The host owns its configuration here; the core only reads it. Dropping it also keeps ruamel.yaml out of the dependency set.
...
# Removed during extraction -- atomic_roundtrip_yaml_save(): persists a whole config-state dict back to the user's file, same CLI concern and same ruamel dependency. Nothing in the core calls it.
