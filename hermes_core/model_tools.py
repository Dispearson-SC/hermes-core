...
from hermes_core.tools.registry import CHECK_FN_CACHE_BYPASS, check_fn_cache_scope, discover_builtin_tools, registry, tool_error
from hermes_core.tools.registry import _MAX_TOOL_ERROR_CHARS as _TOOL_ERROR_MAX_LEN
from hermes_core.toolsets import resolve_toolset, validate_toolset
from hermes_core.tools.arg_coercion import coerce_tool_args
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    try:
        from hermes_core.agent.delegation_context import is_dispatcher_owned_worker_context
...
def _tool_defs_cache_key(
    ...
    if profile_scope == CHECK_FN_CACHE_BYPASS:
        return None
    try:
        from hermes_core.seams.config import get_config_path
...
def _apply_toolset_selection(tools: set, names: List[str], quiet_mode: bool, *, disable: bool) -> None:
    ...
    from hermes_core.toolsets import bundle_non_core_tools, get_toolset
...
def _rewrite_execute_code(td: Dict[str, Any], available: set) -> Optional[Dict[str, Any]]:
    ...
    from hermes_core.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
...
def _discord_rewriter(schema_fn_name: str):
    ...
    def _rewrite(td: Dict[str, Any], available: set) -> Optional[Dict[str, Any]]:
        try:
            from hermes_core.tools import discord_tool as _dt
...
def _compute_tool_definitions(enabled_toolsets: Optional[List[str]] = None, disabled_toolsets: Optional[List[str]] = None,
    ...
    try:
        from hermes_core.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from hermes_core.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    try:
        ...
        if not model_id:
            return 0
        from hermes_core.agent.model_metadata import get_cached_context_length, get_model_context_length
...
def _tool_result_observer_fields(tool_name: str, result: Any) -> tuple[str, Optional[str], Optional[str]]:
    ...
    try:
        ...
        pass
    try:
        from hermes_core.agent.display import _detect_tool_failure
...
def _dispatch_bridge_tool(function_name: str, function_args: Dict[str, Any],
    ...
    """
    try:
        from hermes_core.tools import tool_search as ts
    except Exception:
        return None
...
def handle_function_call(
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from hermes_core.tools.file_tools_read_tracking import notify_other_tool_call
