...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from hermes_core.runtime.auth import _default_verify
...
def _print_login_success(
    ...
    if show_auth_state:
        from hermes_core.seams.paths import display_hermes_home as _dhh
...
def _offer_existing_oauth_credentials(
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    """
    from hermes_core.runtime.auth import _update_config_for_provider
    try:
        existing = resolve()
