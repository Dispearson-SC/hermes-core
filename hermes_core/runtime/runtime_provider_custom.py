...
from hermes_core.runtime.providers import custom_provider_aliases, custom_provider_slug
from hermes_core.utils import base_url_hostname

logger = logging.getLogger("hermes_core.runtime.runtime_provider")
...
def _rp():
    ...
    import hermes_core.runtime.runtime_provider as origin
    return origin
...
def is_routable_provider(provider: Optional[str]) -> bool:
    ...
    if name.lower() == "custom":
        return False
    try:
        from hermes_core.runtime.providers import resolve_provider_full
        rp = _rp()
        config = rp.load_config()
...
def _resolve_llamacpp_runtime(requested_provider: str, explicit_api_key: Optional[str]) -> Dict[str, Any]:
    ...
    rp = _rp()
    try:
        from hermes_core.runtime.local_runtime.endpoint import resolve_llamacpp_endpoint
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if key_cmd and not rp.has_usable_secret(explicit_key):
        from hermes_core.agent.command_token_source import build_command_token_provider
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from hermes_core.runtime.models import normalize_opencode_base_url, opencode_model_api_mode
