...
from hermes_core.utils import atomic_replace, fast_safe_load
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        from hermes_core.agent.secret_scope import _is_global_env, load_env_file
        from hermes_core.agent.secret_sources.registry import apply_all
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    try:
        from hermes_core.agent.secret_sources.registry import get_source
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from hermes_core.seams.config import _sanitize_env_lines
    except ImportError:
    ...
    *,
    ...
    if is_multiplex_active() and get_hermes_home_override() is not None:
        ...
        if load_external_secrets:
            from hermes_core.runtime import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_hermes_home().resolve():
            return
        from hermes_core.seams.config import apply_terminal_config_to_env
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    if home_path == _process_hermes_home():
        try:
            from hermes_core.seams.config import read_raw_config
...
def _process_hermes_home() -> Path:
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    """
    try:
        from hermes_core.seams.paths import get_process_hermes_home
