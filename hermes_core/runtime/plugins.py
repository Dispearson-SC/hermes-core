...
from hermes_core.seams.paths import get_hermes_home, hermes_home_key
from hermes_core.registration_lifecycle import replacement_coordinator
from hermes_core.utils import env_var_enabled
from hermes_core.seams.config import load_config_readonly
from hermes_core.runtime.middleware import VALID_MIDDLEWARE
from hermes_core.runtime.plugin_capabilities import plugin_capability_granted
from hermes_core.runtime.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from hermes_core.runtime.plugins_manifest import (  # noqa: F401 — re-exported
...
from hermes_core.runtime.plugins_discovery import (  # noqa: F401 — re-exported
...
from hermes_core.runtime.plugins_loader import (
...
from hermes_core.runtime.plugins_dispatch import (  # noqa: F401 — re-exported
...
from hermes_core.runtime.plugins_ledger import PluginLedgerMixin, PluginRegistration
from hermes_core.runtime.plugins_state import (
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        from hermes_core.runtime.platform_actions import PlatformActions
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from hermes_core.agent.plugin_llm import PluginLlm
    ...
    def subagent_lifecycle(self) -> Any:
        ...
        from hermes_core.agent.subagent_lifecycle import SubagentLifecycleService, get_active_subagent_parent
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from hermes_core.runtime.approval_transport import RegisteredApprovalTransport
    ...
    def call_mcp(
        ...
        from hermes_core.tools.mcp_tool_handlers import _make_tool_handler
    ...
    def register_command(
        ...
        with suppress(Exception):  # reject if it conflicts with a built-in command
            from hermes_core.runtime.commands import resolve_command
    ...
    def register_context_engine(self, engine) -> Optional[PluginRegistration]:
        ...
        if self._manager._context_engine is not None:
            logger.warning("Plugin '%s' tried to register a context engine, but one is "
                           ...
                           self.manifest.name)
            return
        from hermes_core.agent.context_engine import ContextEngine
    ...
    def register_context_reference(self, provider) -> None:
        ...
        rejected."""
        from hermes_core.agent.context_references import (
    ...
    def register_memory_provider(self, provider) -> None:
        ...
        from hermes_core.agent.memory_provider import MemoryProvider
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        from hermes_core.runtime.dashboard_auth import DashboardAuthProvider
        from hermes_core.runtime.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from hermes_core.runtime.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_redaction_patterns(self, patterns) -> int:
        ...
        and are skipped."""
        from hermes_core.agent.redact import register_redaction_patterns as _register
        try:
    ...
    def register_skill(
        ...
        from hermes_core.agent.skill_utils import _NAMESPACE_RE
        if ":" in name:
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "hermes_core.agent.secret_sources.registry",
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def _re_register_config_hooks_after_force(self) -> None:
        ...
        for label, module_name in (("shell-hook", "hermes_core.agent.shell_hooks"),
                                   ("outbound-webhook", "hermes_core.agent.outbound_webhooks")):
            try:
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            from hermes_core.agent.secret_sources.registry import list_plugin_sources
            from hermes_core.runtime.env_loader import load_hermes_dotenv, reset_secret_source_cache
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        ...
        """
        try:
            from hermes_core.runtime.plugin_compat import compat_report
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from hermes_core.runtime.agent_plugins import _discover_mcp
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from hermes_core.runtime.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _get_pre_tool_call_directive_details(
    ...
    from hermes_core.runtime.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def _resolve_block_from_details(
    ...
    if details.action != "approve":
        return None
    try:
        from hermes_core.tools.approval import request_tool_approval
        from hermes_core.tools.approval_context import reset_current_observability_context, set_current_observability_context
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('hermes_core.runtime.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('hermes_core.runtime.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('hermes_core.runtime.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('hermes_core.runtime.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('hermes_core.runtime.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('hermes_core.runtime.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('hermes_core.seams.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('hermes_core.runtime.plugins_dispatch', 'format_system_prompt_section'),
