...
from hermes_core.runtime.models_reasoning_caps import _seed_reasoning_caps
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from hermes_core.runtime.models import _urlopen_model_catalog_request
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    from hermes_core.runtime.models import _HERMES_USER_AGENT
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        from hermes_core.runtime.auth import _nous_inference_env_override
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    try:
        from hermes_core.runtime.nous_account import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from hermes_core.runtime.models import _pricing_profile_key
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from hermes_core.runtime.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from hermes_core.runtime.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def get_pricing_for_provider(
    ...
    from hermes_core.runtime.models import normalize_provider
    normalized = normalize_provider(provider)
    if cached_only:
...
def _fireworks_pricing_from_models_dev(*, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    result: dict[str, dict[str, str]] = {}
    try:
        from hermes_core.agent.models_dev import _get_provider_models
...
def _fetch_deepinfra_pricing(timeout: float = 5.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    helper)."""
    from hermes_core.runtime.models import _fetch_deepinfra_models_by_tag
