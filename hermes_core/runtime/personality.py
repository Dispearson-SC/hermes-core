...
# Removed during extraction -- persist_personality(): writes the chosen persona back into the user's config.yaml. Configuration belongs to the host here -- the core reads it and never edits it -- and this was the last caller of the comment-preserving YAML writer dropped from utils.py. Reading and normalising a persona still works; a host that wants to remember a choice stores it wherever it keeps its own settings.
