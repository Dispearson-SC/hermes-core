...
from hermes_core.seams.paths import INDICATOR_STYLES
...
def _resolve_config_gates() -> set[str]:
    ...
    if not gated:
        return set()
    try:
        from hermes_core.seams.config import cfg_get, read_raw_config
        cfg = read_raw_config()
    except Exception:
        return set()
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    try:
        from hermes_core.runtime.plugins import get_plugin_commands
...
def _collect_gateway_skill_entries(
    ...
    try:
        from hermes_core.agent.skill_commands import get_skill_commands
        from hermes_core.tools.skills_tool import SKILLS_DIR
