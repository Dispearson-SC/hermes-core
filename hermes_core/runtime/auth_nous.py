...
from hermes_core.runtime.auth_codex import _pool_entries
from hermes_core.runtime.auth_constants import (
...
if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from hermes_core.runtime.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "hermes_core.runtime.auth").
logger = logging.getLogger("hermes_core.runtime.auth")
...
def _nous_inference_env_override() -> Optional[str]:
    ...
    """
    from hermes_core.runtime.auth import _optional_base_url
...
def _nous_invoke_jwt_status(
    ...
    from hermes_core.runtime.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from hermes_core.runtime.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from hermes_core.runtime.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from hermes_core.runtime.auth import _nonempty_str
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    """
    from hermes_core.runtime.auth import _file_lock
    try:
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from hermes_core.runtime.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    """
    from hermes_core.runtime.auth import _nonempty_str, _write_private_file_atomic
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    """
    from hermes_core.runtime.auth import _auth_file_path
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from hermes_core.runtime.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from hermes_core.runtime.auth import _coerce_ttl_seconds
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    ...
    it.
    """
    from hermes_core.runtime.auth import _save_active_provider_state, _write_shared_nous_state
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from hermes_core.runtime.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from hermes_core.runtime.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    """
    from hermes_core.runtime.auth import _parse_iso_timestamp
    try:
        from hermes_core.seams.credential_pool import load_pool
        pool = load_pool("nous")
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from hermes_core.runtime.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from hermes_core.runtime.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    """
    from hermes_core.runtime.auth import _prompt_model_selection
    ...
    from hermes_core.runtime.models_pricing import (
    ...
    if model_ids:
        from hermes_core.runtime.nous_account import nous_policy_notice
...
def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    ...
    """
    from hermes_core.runtime.auth import _prompt_yes_no, _read_shared_nous_state
    shared = _read_shared_nous_state()
    if not shared:
        return None
    try:
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    ...
    from hermes_core.runtime.auth import _auth_store_lock, _load_auth_store, _save_auth_store
