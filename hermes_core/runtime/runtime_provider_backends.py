...
def _azure_entra_credentials(cfg_entra: Dict[str, Any]) -> Any:
    ...
    try:
        from hermes_core.agent.azure_identity_adapter import SCOPE_AI_AZURE_DEFAULT, EntraIdentityConfig, build_token_provider
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from hermes_core.agent.bedrock_adapter import (bedrock_openai_base_url, has_aws_credentials, is_anthropic_bedrock_model,
                                       is_openai_bedrock_model, resolve_aws_auth_env_var, resolve_bedrock_bearer_token,
                                       resolve_bedrock_runtime_region)
    from hermes_core.seams.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
