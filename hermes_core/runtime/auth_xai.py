...
from hermes_core.runtime.auth_codex import _load_auth_store_maybe_locked, _refresh_payload_access_token
...
from hermes_core.utils import env_float
...
def _xai_oauth_state_from_store(auth_store: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    from hermes_core.runtime.auth import _load_provider_state
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    from hermes_core.runtime.auth import _load_global_auth_store
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    """
    from hermes_core.runtime.auth import _global_auth_file_path, _persist_provider_state_to_store
...
def _save_xai_oauth_tokens(
    ...
    """
    from hermes_core.runtime.auth import _auth_store_lock, _global_auth_file_path, _load_auth_store, _load_provider_state_with_source, _same_path, _save_auth_store, _store_provider_state, _utc_now_z, _write_through_xai_oauth_to_global_root
...
def refresh_xai_oauth_pure(
    ...
    from hermes_core.runtime.auth import _nonempty_str, _utc_now_z, _xai_oauth_discovery
    del access_token
...
def _refresh_xai_oauth_tokens(
    ...
    from hermes_core.runtime.auth import _load_auth_store, _load_provider_state, refresh_xai_oauth_pure
    try:
...
def _quarantine_xai_oauth_tokens(exc: AuthError) -> None:
    ...
    """
    from hermes_core.runtime.auth import _last_auth_error_marker, _load_auth_store, _load_provider_state, _save_auth_store, _store_provider_state
    try:
...
def resolve_xai_oauth_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from hermes_core.runtime.auth import _auth_store_lock, _is_terminal_xai_oauth_refresh_error, _refresh_xai_oauth_tokens, _xai_oauth_discovery
...
def _login_xai_oauth(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    from hermes_core.runtime.auth import _is_remote_session, _offer_existing_oauth_credentials, _print_login_success, _update_config_for_provider, _xai_oauth_device_code_login, resolve_xai_oauth_runtime_credentials, unsuppress_credential_source
    del pconfig
...
def _xai_oauth_poll_device_token(
    ...
    from hermes_core.runtime.auth import _poll_device_token_generic
...
def _xai_oauth_device_code_login(*, timeout_seconds: float = 20.0, open_browser: bool = True) -> Dict[str, Any]:
    from hermes_core.runtime.auth import _can_open_graphical_browser, _is_remote_session, _print_device_code_instructions, _utc_now_z, _xai_oauth_discovery, _xai_oauth_poll_device_token
